...
-- 2	TCourseBooks					TCourses					intCourseID
-- 3	TCourseBooks					TBooks						intBookID
...
WHERE
	 TCS.intStudentID		= TS.intStudentID
...
-- 6	TFlightFlightAttendants			TFlights					intFlightID
-- 7	TFlightFlightAttendants			TFlightAttendants			intFlightAttendantID
...
INSERT INTO TFlightFlightAttendants ( intFlightID, intFlightAttendantID, strFlightAttendantRole ) 
VALUES		 ( 1, 1, 'Chief Attendant' )
			,( 1, 2, 'Attendant' )
			,( 2, 3, 'Chief Attendant' )
			,( 2, 4, 'Attendant' )
			,( 3, 5, 'Chief Attendant' )
...
FROM
	...
	,TPilots			AS TPi
	,TAirplanes			AS TA
	,TFlightPilots		AS TFPi

WHERE
	 TF.intAirplaneID   = TA.intAirplaneID
AND	 TFPi.intFlightID	= TF.intFlightID
AND  TFPi.intPilotID		= TPi.intPilotID
AND	 TFPi.strPilotRole = 'Pilot'
...
FROM
	 TPassengers		AS TPas
	,TFlights			AS TF
	,TFlightPassengers  AS TFPas

WHERE
	 TFPas.intPassengerID = TPas.intPassengerID
AND  TFPas.intFLightID = TF.intFlightID
